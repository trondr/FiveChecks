...
namespace Compliance.Notifications.Commands.CheckDiskSpace
{
    ...
    {
        public static async Task<Result<int>> CheckDiskSpace(string[] args)
        {
            ...
            else
            {
                ...
                var toastContent = await ActionSnoozeDismissToastContent.CreateToastContent(title, image, companyName, content, content2, action).ConfigureAwait(true);
